...
- "STAR DIMANCHE" (`RAW_BJ_S_DIM`) — **CORRECTION (06-07/09/2026, en 2 temps)** : une première hypothèse ("= copie de Star 18h le dimanche, 1 ligne") était fausse et a été corrigée après retour de l'utilisateur avec une capture de référence montrant 3 lignes par dimanche. Le format correct : **3 tirages par dimanche (Star 18h, puis Star 14h, puis Star 11h, dans cet ordre décroissant)**, reconstruits à partir de `RAW_BJ_S11`/`RAW_BJ_S14`/`RAW_BJ_S18` pour chaque dimanche où ces tables ont une entrée.
  - **2e correction (07/09/2026)** : l'hypothèse de repli initiale ("1 seule ligne pour les dimanches antérieurs à ~21/05/2023") était ELLE AUSSI fausse — l'utilisateur a fourni une capture complète et faisant autorité de la source "STAR DIMANCHE 2023" (N° 1 à 72) montrant en réalité **2 tirages par dimanche dès le 01/01/2023**, un 3e apparaissant seulement à partir du **16/07/2023** (pas le 21/05/2023 comme supposé — 21/05/2023 est juste la date où les tables individuelles `RAW_BJ_S14`/`RAW_BJ_S18` de l'app ont commencé à exister, coïncidence trompeuse). Les 20 dimanches du 01/01/2023 au 14/05/2023 n'avaient donc dans l'app qu'UNE seule ligne (celle qui existait déjà, confirmée identique au N° impair de la source de référence) — le 2e tirage manquant de chacun de ces 20 dimanches a été retranscrit mot pour mot depuis la capture fournie et inséré à sa place chronologique. Total : 531 → **551 lignes**, réindexées en IDs séquentiels 1→N (1 = 01/01/2023, 551 = 06/09/2026).
  - **Numérotation externe adoptée (07/09/2026)** : l'utilisateur a fourni le fichier complet `Star_Dimanche_2023_2026.xlsx` (feuille "Complet 2023-2026", N° 660 à 1210, 551 lignes) qui fait autorité pour la numérotation. Comparaison ligne à ligne : les 551 valeurs correspondent EXACTEMENT à celles déjà reconstruites dans l'app avec un décalage constant de **+659** (01/01/2023 = 660 ↔ notre ancien id 1 ; 06/09/2026 = 1210 ↔ notre ancien id 551) — une seule différence trouvée (14/09/2025 : le fichier de référence indique un total de 268, mathématiquement faux pour 86+56+44+52+28=266 ; la valeur 266, correcte, a été conservée). `RAW_BJ_S_DIM` a donc été renuméroté avec ce décalage +659 (plus besoin d'historique pré-2023 : le fichier fourni couvrait déjà tout le périmètre 2023-2026 nécessaire).
  - Elle n'alimente ni `RAW_TOUTBENIN` ni `RAW_BENIN_ALL_JEU` (S11/S14/S18 y sont déjà présents sous leur propre nom).
