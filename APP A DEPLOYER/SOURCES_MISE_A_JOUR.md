...
- "STAR DIMANCHE" (`RAW_BJ_S_DIM`) — **CORRECTION (06/09/2026)** : une première hypothèse ("= copie de Star 18h le dimanche, 1 ligne") était fausse et a été corrigée après retour de l'utilisateur avec une capture de référence montrant 3 lignes par dimanche. Le format correct : **3 tirages par dimanche (Star 18h, puis Star 14h, puis Star 11h, dans cet ordre décroissant)**, reconstruits à partir de `RAW_BJ_S11`/`RAW_BJ_S14`/`RAW_BJ_S18` pour chaque dimanche où ces 3 tables ont une entrée. Pour les dimanches antérieurs à ~21/05/2023 (avant que S11/S14/S18 existent), l'ancienne ligne unique est conservée telle quelle (fallback) pour ne perdre aucun historique — 20 dimanches dans ce cas sur 193 au total (06/09/2026 : 531 lignes reconstruites). Script de reconstruction : fusionner les 3 tables par date, filtrer les dimanches (`date.weekday()==6`), construire jusqu'à 3 lignes par dimanche, réindexer en IDs séquentiels 1→N. Elle n'alimente ni `RAW_TOUTBENIN` ni `RAW_BENIN_ALL_JEU` (S11/S14/S18 y sont déjà présents sous leur propre nom).
